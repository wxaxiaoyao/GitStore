...
class Store {
	...
	// 保存文件
	async saveFile({repopath, filepath, content = "", message, ref, committer = {}, encoding = "base64"}) {
		assert(repopath);
		assert(filepath);
		...
		// 提交失败删除文件
		if (!oid) {
			_fs.unlinkSync(this.getObjectPath({repopath, sha: id.tostrS()}));
	...
	// 通过sha获取tree
	async getTreeById({repo, id, recursive,  prefix = ""}) {
